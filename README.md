![header](https://capsule-render.vercel.app/api?type=waving&color=0:1a1b27,50:414868,100:bf91f3&height=200&section=header&text=Product%20Engineer:%20Han%20Tae-young&fontSize=40&fontColor=70a5fd&fontAlignY=35&animation=fadeIn)

[![GitHub Stats](https://github-readme-stats-chi-one-57.vercel.app/api?username=xodudgks&show_icons=true&theme=tokyonight&v=2)](https://github.com/anuraghazra/github-readme-stats)
--- 

![header](https://capsule-render.vercel.app/api?type=waving&color=0:1a1b27,50:414868,100:bf91f3&height=200&section=header&text=Product%20Engineer:%20Han%20Tae-young&fontSize=40&fontColor=70a5fd&fontAlignY=35&animation=fadeIn)

## 🎓 Education
- **Soongsil University** — Global Media, IT College (Class of 2024)

## 📚 Tech Stack

**Frontend**
`JavaScript` `TypeScript` `React` `Next.js` `TanStack Query` `Zustand` `Tailwind CSS`

**Backend & Infra**
`Node.js` `NestJS` `Supabase` `PostgreSQL` `Vercel`

**Tools**
`Git` `GitHub Actions` `Figma` `PostHog`

## 🏛️ Activities

| 소속 | 직책 | 기간 |
|---|---|---|
| 숭실대학교 IT대학 학생회 | 기획국원 (17대 Commit) | 2024.03 ~ 2024.12 |
| 숭실대학교 IT대학 학생회 | 기획국장 (18대 Edit) | 2025.01 ~ 2025.12 |
| 숭실대학교 글로벌미디어학부 소모임 슈퍼배드 | 회장 | 2024.08 ~ 진행중 |
| 숭실대학교 IT대학 X AI대학 소모임 SUMMIT | 부회장 | 2026.01 ~ 진행중 |
| 숭실대학교 교내 컴퓨터 동아리 SSCC 44기 | 회원 | 2026.03 ~ 진행중 |

## 🫂 Projects

| 프로젝트 | 역할 | 기간 | 설명 |
|---|---|---|---|
| A:SSU | PM | 2026.02 ~ 진행중 | 학생·업체·학생회를 연결하는 대학생 제휴 관리 플랫폼 |
| Chwihap | Front-end | 2026.06 ~ 진행중 | 올인원 취업 준비 관리 서비스 |
| Summit Concert Web | Full-Stack | 2026.04 ~ 진행중 | 써밋 정기공연 통합 웹서비스 |
| Pickhabju | 마케팅 | 2025.08 ~ 2026.06 | 합주실 통합 검색 서비스 |

## 🏆 Awards & Honors

| 프로젝트 | 수상 내역 | 날짜 |
|---|---|---|
| A:SSU | 숭실대학교 컴퓨터학부 소프트웨어공모전 총장상 | 2026.08 |
| UX/UI | 숭실대학교 교내 우수학습 공모전 UX/UI 디자인 3등 | 2026.02 |
| Unithon | 숭실대학교 IT대학 학장 공로상 | 2025.08 |
| TEAMO | 숭실대학교 소프트웨어학부 소프트웨어공모전 은상 | 2025.08 |
| Memorium | 글로벌미디어학부 소모임 연합 해커톤 Wish 장려상 | 2024.04 |
